...
function GoalsTracking() {
  ...
  const handleEditGoal = (goal) => {
    ...
    setDeadline(goal.deadline ? new Date(goal.deadline.seconds * 1000).toISOString().split('T')[0] : "");
  ...
  const calculateGoalStatus = (goal) => {
    ...
    const progressPercentage = Math.round((goal.progress / goal.targetAmount) * 100);
      ...
      goals.forEach(goal => {
        ...
        if (goal.deadline) {
          ...
          if (daysRemaining <= 2 && daysRemaining > 0) {
            ...
            // In-app notification
            toast.warning(`${goal.title} deadline is in ${daysRemaining} days!`);
          }
        }
      });
    };
  ...
  if (isLoading) {
    return <div className="flex justify-center items-center h-64">Loading your goals...</div>;
  }

  if (!uid) {
    return <div className="flex justify-center items-center h-64">Please log in to view your goals.</div>;
  }

  return (
    <div className="p-6">
      <div className="pt-[5vh] font-semibold text-3xl flex justify-between items-center">
        <span>Goals Tracking</span>
        <button 
          onClick={() => setShowForm(!showForm)} 
          className="bg-[#2E8B57] text-white text-base sm:text-xl px-2 py-1 rounded-md hover:bg-[#1e6b47] transition duration-300"
        >
      ...
      {showForm && (
        <div className="mt-6 p-6 border rounded-md shadow-sm bg-white">
          <h3 className="text-xl font-medium mb-4">{editingGoalId ? "Edit Goal" : "Create New Goal"}</h3>
          <form onSubmit={handleAddGoal} className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            {(userAllergies.length > 0 || userChronicDiseases.length > 0) && (
              <div className="md:col-span-2 p-4 bg-blue-50 rounded-md mt-2">
                <h4 className="text-sm font-medium mb-2">Your Health Factors (Will be considered for this goal)</h4>
                ...
                {userChronicDiseases.length > 0 && (
                  <div>
                    ...
                    <span className="text-sm">{userChronicDiseases.join(", ")}</span>
                  </div>
                )}
              </div>
            )}
        ...
        {goals.length === 0 ? (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {goals.map((goal) => {
              ...
              const progressPercentage = Math.min(Math.round((goal.progress / goal.targetAmount) * 100), 100);
              
              return (
                <div key={goal.id} className={`border rounded-lg overflow-hidden ${goal.completed ? 'border-green-200 bg-green-50' : 'border-gray-200'}`}>
                  <div className="p-4">
                    ...
                    <div className="flex justify-between items-center text-sm text-gray-600 mb-1">
                      <span>Progress: {goal.progress} of {goal.targetAmount}</span>
                    ...
                    <div className="grid grid-cols-2 gap-4 text-sm">
                      ...
                      <div>
                        ...
                        <span>{goal.frequency.value} per {goal.frequency.period}</span>
                      </div>
                      <div>
                  ...
                  <div className="bg-gray-50 px-4 py-3 flex justify-between items-center border-t">
                    <div className="flex gap-2">
                      <button
                        onClick={() => updateGoalProgress(goal.id, Math.max(0, goal.progress - 1))}
...
//       <div className="pt-[5vh] font-semibold text-3xl">Goals Tracking</div>
